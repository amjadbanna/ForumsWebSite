...
import { getStats, adminDeletePost, setUserStatus } from "../../controllers/admin.js"
...
const user = { id: "user-1", username: "alice", password: "password123", role: "user" as const, status: "active" as const }
...
describe("getStats", () => {
  it("returns site stats", async () => {
    const stats = { totalUsers: 1, totalPosts: 1, totalComments: 1, totalLikes: 1 }
...
describe("adminDeletePost", () => {
  it("returns 404 when post does not exist", async () => {
    ...
    const res = mockRes()
    await adminDeletePost({ params: { id: "post-1" }, user: admin } as unknown as Request, res)
...
describe("setUserStatus", () => {
  it("returns 400 for an invalid status", async () => {
    const res = mockRes()
    await setUserStatus({ body: { status: "banned" }, params: { id: "user-1" }, user: admin } as unknown as Request, res)
    expect(res.status).toHaveBeenCalledWith(400)
  })

  it("returns 403 when admin tries to change their own status", async () => {
    jest.mocked(userRepo.findUserById).mockResolvedValue({ ...user, id: "admin-1" })
    const res = mockRes()
    await setUserStatus({ body: { status: "blocked" }, params: { id: "admin-1" }, user: admin } as unknown as Request, res)
    expect(res.status).toHaveBeenCalledWith(403)
  })

  it("updates the status successfully", async () => {
    jest.mocked(userRepo.findUserById).mockResolvedValue(user)
    jest.mocked(userRepo.updateUserStatus).mockResolvedValue({ ...user, status: "blocked" })
    const res = mockRes()
    await setUserStatus({ body: { status: "blocked" }, params: { id: "user-1" }, user: admin } as unknown as Request, res)
    expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ message: 'User status updated to "blocked"' }))
  })
})
